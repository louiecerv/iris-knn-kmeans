...
def app():
    ...
    if "new_clusters" not in st.session_state:
        st.error('You must generate random clusters. Go to the Random Clusters page.')

    text = """Click to begin the unsupervised clustering of the data using k-means."""
    st.write(text)

    if st.button('Begin Clustering Task'):
    ...
    text = """We can visualize the clusters by trying different values of k."""
    st.write(text)
